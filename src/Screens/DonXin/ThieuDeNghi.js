...
export default function PhieuDeNghi({ navigation }) {
    ...
    const PhieuDeNghi = list.filter((item) => item.type === 'Phiếu đề nghị')
    const handleSubmit = () => {
        const date = new Date()
        console.log(date.getDate(), date.getSeconds())
        const data = {
            name: 'quy',
            type: 'Phiếu đề nghị',
            LiDoNghi: liDoDeNghi,
            CamKet: noiDung,
